<template>
  <div id="clockRow">
    <div class="top">
      <h1>Zit er een 5 in de Klok?</h1>
      <div id="searchClock">
        <input
          id="search"
          type="search"
          placeholder="Zoek Klok..."
          v-model="zoekClock"
          :autofocus="'autofocus'"
        />
      </div>
    ...
    <div class="bottom">
      <div class="clock-list">
        <!-- <div v-for="(timezone, i) in timezones" :key="i"> -->
        <div v-for="(timezone, i) in searchTimeZones" :key="i">
          <DigiClock :timezone="timezone" />
        </div>
      </div>
    </div>
  </div>
  ...
  computed: {
    searchTimeZones: function() {
      return this.timezones.filter(timezone => {
        return (
          this.zoekClock.length === 0 ||
          timezone.label.toLowerCase().includes(this.zoekClock.toLowerCase()) ||
          timezone.tzCode.toLowerCase().includes(this.zoekClock.toLowerCase())
        );
      });
    }
  }
...
@import "../styles/components/_clockrow.scss";
</style>
